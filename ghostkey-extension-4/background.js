...
async function handleAttempt(data, tab) {
  ...
  if (!profile) return; /* not set up yet — ignore */
  ...
  /* alert on low trust */
  if (!trusted) {
    chrome.notifications.create(`gk-${Date.now()}`, {
      type:    'basic',
      iconUrl: 'icon.png',
  ...
  /* auto-open (or focus) the popup window after every sign-in */
  openPopupWindow();
}
...
async function openPopupWindow() {
  ...
  /* reuse an already-open LowK popup window if one exists */
  ...
  /* otherwise open a fresh popup window */
